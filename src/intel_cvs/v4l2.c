				...
				struct v4l2_async_connection *asc)
{
	...
	/*
	 * The IPU ISYS only creates sub-device nodes from its own notifier
	 * callbacks. The sensor binds to this bridge instead, so create its
	 * node here: it has just been added to the v4l2_device.
	 */
	if (!ctx->subdev.v4l2_dev)
		return 0;

	return v4l2_device_register_subdev_nodes(ctx->subdev.v4l2_dev);
}
