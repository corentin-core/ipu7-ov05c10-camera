...
static const struct icvs_device_quirk cvs_quirk_table[] = {
	...
	{ 0x06CB, 0x0701, ICVS_SKIP_FW_RESET |
			  ...
			  /*
			   * ICVS_HOST_PRIV_CTRL dropped: it claims privacy
			   * LED gating from the firmware, which this driver
			   * then never exercises, leaving the LED dark.
			   * Without it the chip drives the LED itself.
			   */
