...
Verified on a **Dell Pro 16 Plus (PB16250)**, Ubuntu 24.04, HWE kernel 7.0. Other Dell Pro models of the same generation use the same combination.
...
```bash
lspci | grep -i 'Device 645d'                 # an Intel IPU7?
...
|---|---|---|
| 26.04 and later | 0.7 in the archive | `sudo apt install libcamera-tools`, nothing else |
...
With Secure Boot on, enrol the DKMS signing key once, then reboot:

```bash
sudo mokutil --import /var/lib/dkms/mok.pub
```

Finally, **in Firefox**, set `media.webrtc.camera.allow-pipewire` to `true` in `about:config`. Its V4L2 path crashes on this device, in both 155 and 156, snap and deb alike. Chrome uses V4L2 directly and is fine.

## Building libcamera, for releases before 26.04

```bash
sudo apt install -y git meson ninja-build pkg-config python3-yaml python3-ply \
...
Then point the service at the plugin, since `/usr/local` is outside GStreamer's default search path:

```bash
sudo sed -i 's|^#GST_PLUGIN_PATH|GST_PLUGIN_PATH|' /etc/default/ipu7-camera-ondemand
...
The colour profile `ov05c10.yaml`, shipped by the package, enables the colour correction matrix that the uncalibrated profile leaves out. Saturation goes from 18 to 27 on our measurements. The matrix is **not measured against a colour target**: it is a reasoned boost whose rows sum to 1, so it does not move the white balance, which is already correct.
...
All three patches are documented in [`src/PROVENANCE.md`](src/PROVENANCE.md). Two of them are **not upstream**: that file is where to look first when a kernel update breaks the camera.
...
A hard teardown of the stream used to cause the same lockup. That is handled: the supervisor stops the pipeline with `SIGINT`, the only signal `gst-launch` tears down cleanly, and never kills it outright. A camera left running beats a dead bridge. The signature of a failed teardown is `stream stop time out` in `dmesg`.

Two transient `Bulk` errors show up at each power-up. No observed consequence.
...
**`exclusive_caps=1` is mandatory**, because browsers skip any device that also advertises the output capability. But the capture capability only appears after the *first write*, hence the black priming frame at startup.
...
`-110` errors in `dmesg`: full power off.

## Also needed: a working v4l2loopback

Ubuntu's `v4l2loopback-dkms` 0.12.7 does not build against a 7.0 kernel (`too few arguments to function 'v4l2_fh_add'`). Use upstream 0.15.4 or later:
